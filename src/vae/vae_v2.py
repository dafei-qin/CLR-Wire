...
# The second version on Dec. 2025, with the model output the u_closed and v_closed flags.
...
class SurfaceVAE(nn.Module):
    def __init__(self, 
        ...
        self.is_closed_classifier = nn.Linear(latent_dim, 2)
    ...
    def forward(self, params, surface_type):
        ...
        class_logits, surface_type_pred, is_closed_logits, is_closed = self.classify(z)
        recon, mask = self.decode(z, surface_type)
        return recon, mask, class_logits, is_closed_logits, mu, logvar
