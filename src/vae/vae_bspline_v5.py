...
class BSplineVAE(nn.Module):
    def __init__(self, 
        ...
        # 输出潜空间参数
        self.fc_mu = nn.Linear(embd_dim * num_query, embd_dim * num_query // 8)
        self.fc_logvar = nn.Linear(embd_dim * num_query, embd_dim * num_query // 8)
        
        # Decoder
        # TODO

        
        self.deg_head_u = nn.Linear(embd_dim * num_query // 8, max_degree)  # max_degree=3 → classes: deg1,deg2,deg3
        self.deg_head_v = nn.Linear(embd_dim * num_query // 8, max_degree)

        self.peri_head_u = nn.Linear(embd_dim * num_query // 8, 1) # 0 or 1
        self.peri_head_v = nn.Linear(embd_dim * num_query // 8, 1)

        self.knots_num_head_u = nn.Linear(embd_dim * num_query // 8, max_num_u_knots)
        self.knots_num_head_v = nn.Linear(embd_dim * num_query // 8, max_num_v_knots)
        
        self.token_proj_u = NonlinearMLP(embd_dim * num_query // 8, embd_dim, max_num_u_knots * embd_dim)
        self.token_proj_v = NonlinearMLP(embd_dim * num_query // 8, embd_dim, max_num_v_knots * embd_dim)
        ...
        self.token_proj_poles = NonlinearMLP(embd_dim * num_query // 8, embd_dim, embd_dim)
