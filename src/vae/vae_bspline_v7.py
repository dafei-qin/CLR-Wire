...
# In V7 we try to accelerate the training by vectorizing the computation of the positional encodings.
# We also change the output of the fuser to be the concatenation of all the tokens, instead of the last token. Then we use a MLP to project the concatenated tokens to the latent space.
...
def make_coord_mlp(input_dim: int, embed_dim: int, hidden_dim: int = 64) -> nn.Sequential:
    """
    Create a simple MLP that maps normalized coordinates to embeddings.
    Supports any input shape as long as last dim matches input_dim.
    """
...
class BSplineVAE(nn.Module):
    def __init__(self, 
        ...
        # 1D knots的相对位置编码 (使用简化的MLP)
        self.u_knots_pe = make_coord_mlp(input_dim=1, embed_dim=embd_dim, hidden_dim=64)
        self.v_knots_pe = make_coord_mlp(input_dim=1, embed_dim=embd_dim, hidden_dim=64)
        ...
        # 2D poles的相对位置编码 (使用简化的MLP)
        self.poles_pe = make_coord_mlp(input_dim=2, embed_dim=embd_dim, hidden_dim=128)
        ...
        self.fc_mu = nn.Linear(embd_dim * num_query, embd_dim)
        self.fc_logvar = nn.Linear(embd_dim * num_query, embd_dim)
    ...
    def _compute_1d_pe_batch(self, num_elements: torch.Tensor, max_num: int, pe_mlp: nn.Sequential, device: torch.device):
        """
        向量化计算1D PE
        Args:
            num_elements: (B,) 每个sample的元素数量
            max_num: 最大元素数量
            pe_mlp: nn.Sequential，输入(*, 1)输出(*, embd_dim)
        Returns:
            pe: (B, max_num, embd_dim)
            mask: (B, max_num) bool, True表示有效位置
        """
        B = num_elements.shape[0]
        
        # 创建位置索引 (B, max_num)
        pos_indices = torch.arange(max_num, device=device).unsqueeze(0).expand(B, -1)
        
        # 计算每个sample的归一化分母 (B, 1)
        denominators = (num_elements - 1).clamp(min=1).float().unsqueeze(1)
        
        # 归一化坐标 (B, max_num)
        t_normalized = pos_indices.float() / denominators
        
        # 创建mask (B, max_num)
        mask = pos_indices < num_elements.unsqueeze(1)
        
        # 通过MLP (B, max_num, 1) -> (B, max_num, embd_dim)
        pe = pe_mlp(t_normalized.unsqueeze(-1))
    ...
    def _compute_2d_pe_batch(self, num_rows: torch.Tensor, num_cols: torch.Tensor, 
                              max_rows: int, max_cols: int, pe_mlp: nn.Sequential, device: torch.device):
        """
        向量化计算2D PE
        Args:
            num_rows: (B,) 每个sample的行数
            num_cols: (B,) 每个sample的列数
            max_rows, max_cols: 最大行列数
            pe_mlp: nn.Sequential，输入(*, 2)输出(*, embd_dim)
        Returns:
            pe: (B, max_rows, max_cols, embd_dim)
            mask: (B, max_rows, max_cols) bool, True表示有效位置
        """
        B = num_rows.shape[0]
        
        # 创建位置索引网格 (B, max_rows, max_cols)
        row_indices = torch.arange(max_rows, device=device).view(1, -1, 1).expand(B, max_rows, max_cols)
        col_indices = torch.arange(max_cols, device=device).view(1, 1, -1).expand(B, max_rows, max_cols)
        
        # 计算每个sample的归一化分母 (B, 1, 1)
        row_denominators = (num_rows - 1).clamp(min=1).float().view(B, 1, 1)
        col_denominators = (num_cols - 1).clamp(min=1).float().view(B, 1, 1)
        
        # 归一化坐标 (B, max_rows, max_cols)
        u_normalized = row_indices.float() / row_denominators
        v_normalized = col_indices.float() / col_denominators
        
        # 堆叠成坐标对 (B, max_rows, max_cols, 2)
        coords = torch.stack([u_normalized, v_normalized], dim=-1)
        
        # 创建mask (B, max_rows, max_cols)
        mask = (row_indices < num_rows.view(B, 1, 1)) & (col_indices < num_cols.view(B, 1, 1))
        
        # 通过MLP (B, max_rows, max_cols, 2) -> (B, max_rows, max_cols, embd_dim)
        pe = pe_mlp(coords)
    ...
    def encode(self, u_knots, u_mults, v_knots, v_mults, poles, u_degree, v_degree, u_periodic, v_periodic, num_knots_u, num_knots_v, num_poles_u, num_poles_v):
        ...
        # Add positional encoding to knots and mults (向量化计算)
        u_knots_pe, u_knots_pe_mask = self._compute_1d_pe_batch(
            num_knots_u.squeeze(-1), self.max_num_u_knots, self.u_knots_pe, u_knots.device
        )
        v_knots_pe, v_knots_pe_mask = self._compute_1d_pe_batch(
            num_knots_v.squeeze(-1), self.max_num_v_knots, self.v_knots_pe, v_knots.device
        )
        ...
        # Encode poles (向量化计算PE)
        poles_pe, poles_mask = self._compute_2d_pe_batch(
            num_poles_u.squeeze(-1), num_poles_v.squeeze(-1),
            self.max_num_u_poles, self.max_num_v_poles, self.poles_pe, poles.device
        )
        ...
        fuser_output = fuser_seq.reshape(B, -1) # (B, num_query * embd_dim)
    ...
    def decode_knots_mults(self, z, num_knots_u, num_knots_v):
        ...
        # 向量化生成mask
        pos_indices_u = torch.arange(self.max_num_u_knots, device=device).unsqueeze(0)
        pos_indices_v = torch.arange(self.max_num_v_knots, device=device).unsqueeze(0)
        knots_mask_u = pos_indices_u < num_knots_u.view(B, 1)  # (B, max_num_u_knots)
        knots_mask_v = pos_indices_v < num_knots_v.view(B, 1)  # (B, max_num_v_knots)
        knots_padding_mask_u = ~knots_mask_u
        knots_padding_mask_v = ~knots_mask_v
        ...
        # 向量化计算PE
        knots_pe_u, _ = self._compute_1d_pe_batch(
            num_knots_u.view(-1), self.max_num_u_knots, self.u_knots_pe, device
        )
        knots_pe_v, _ = self._compute_1d_pe_batch(
            num_knots_v.view(-1), self.max_num_v_knots, self.v_knots_pe, device
        )

        # Here the pe is already multiplied with mask in the _compute function 
    ...
    def decode(self, z, num_knots_u, num_knots_v, num_poles_u, num_poles_v):
        ...
        # 向量化计算poles PE
        poles_pe_padded, poles_mask = self._compute_2d_pe_batch(
            num_poles_u.view(-1), num_poles_v.view(-1),
            self.max_num_u_poles, self.max_num_v_poles, self.poles_pe, device
        )
        poles_padding_mask = ~poles_mask
        poles_padding_mask = einops.rearrange(poles_padding_mask, 'b h w -> b (h w)')
    ...
    def classify(self, z):
        ...
        # 向量化选择: 根据periodic标志选择对应的num_poles
        pred_periodic_u_flat = pred_periodic_u.view(-1)  # (B,)
        pred_periodic_v_flat = pred_periodic_v.view(-1)  # (B,)
        num_poles_u = torch.where(pred_periodic_u_flat, peri_num_poles_u, non_peri_num_poles_u)
        num_poles_v = torch.where(pred_periodic_v_flat, peri_num_poles_v, non_peri_num_poles_v)
        num_poles_u = num_poles_u.clamp(min=1, max=self.max_num_u_poles)
        num_poles_v = num_poles_v.clamp(min=1, max=self.max_num_v_poles)
