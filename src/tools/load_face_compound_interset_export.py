...
if __name__ == '__main__':
    ...
    for i in range(len(all_faces)):
        all_data.append({'face': all_faces[i], 'edges': []})
        for j in range(0, len(all_faces)):
            if i == j:
                continue
            ...
            if edges is not None:
                for edge in edges:
                    ...
                    all_data[i]['edges'].append(edge)

    # print(all_edges)
    # write_to_step(all_faces, 'assets/to_zeyu/00100001/detokenized_surfaces_idx_0000_interset.step', edges=all_edges)

    # ps.show()
    pickle.dump(all_data, open('assets/to_zeyu/00100001/detokenized_surfaces_idx_0000_interset.pkl', 'wb'))