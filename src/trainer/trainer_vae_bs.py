...
class TrainerBS(BaseTrainer):
    ...
    def create_surface_visualization_bs(self, gt_samples, cross_attention_recon, bspline_recon, control_points, step):
        ...
        for i in range(batch_size):
            ...
            ax_bs = fig.add_subplot(4, batch_size, 2*batch_size+i+1, projection='3d')
            ...
            ax_ctrl = fig.add_subplot(4, batch_size, 3*batch_size+i+1, projection='3d')
        ...
        with torch.no_grad():
            ...
            if isinstance(output, tuple) and len(output) == 2:
                ...
                if data.dim() == 4:  # Surface data (B, H, W, C)
                    ...
                    # latent = model.encode(data_input).latent_dist.mode()
                    
                    # # Extract control points from B-spline branch
                    # if hasattr(model, 'control_point_mlp') and model.control_point_mlp is not None:
                    #     latent_flat = latent.view(bs, -1)
                    #     control_points_flat = model.control_point_mlp(latent_flat)
                    #     control_points = control_points_flat.view(bs, 16, 3)
                    # else:
                    #     # Fallback if MLP not initialized
                    #     control_points = torch.zeros(bs, 16, 3, device=data.device)
                    
                    # Generate grid coordinates for ground truth sampling
                    ...
                    fig = self.create_surface_visualization_bs(
                        gt_samples, cross_attention_recon, bspline_recon, control_point_results, step
                    )
    ...
    def log_loss(self, total_loss, loss_dict, lr, total_norm, step):
        ...
        #         log_dict['recon_loss'] = loss_dict['recon_loss'].item() if hasattr(loss_dict['recon_loss'], 'item') else loss_dict['recon_loss']
        #     if 'kl_loss' in loss_dict:
        #         log_dict['kl_loss'] = loss_dict['kl_loss'].item() if hasattr(loss_dict['kl_loss'], 'item') else loss_dict['kl_loss']
            
        #     # B-spline specific losses
        #     if 'bspline_recon_loss' in loss_dict:
        #         log_dict['bspline_recon_loss'] = loss_dict['bspline_recon_loss'].item() if hasattr(loss_dict['bspline_recon_loss'], 'item') else loss_dict['bspline_recon_loss']
        #     if 'bspline_control_point_loss' in loss_dict:
        #         log_dict['bspline_control_point_loss'] = loss_dict['bspline_control_point_loss'].item() if hasattr(loss_dict['bspline_control_point_loss'], 'item') else loss_dict['bspline_control_point_loss']
        ...
        # if isinstance(loss_dict, dict):
        #     if 'recon_loss' in loss_dict:
        #         loss_str += f' recon: {loss_dict["recon_loss"]:.3f}'
        #     if 'bspline_recon_loss' in loss_dict:
        #         loss_str += f' bs_recon: {loss_dict["bspline_recon_loss"]:.3f}'
        #     if 'kl_loss' in loss_dict:
        #         loss_str += f' kl: {loss_dict["kl_loss"]:.3f}'
        #     if 'bspline_control_point_loss' in loss_dict:
        #         loss_str += f' bs_ctrl: {loss_dict["bspline_control_point_loss"]:.3f}'
    ...
    def train(self):
        ...
        while step < self.num_train_steps:
            ...
            if self.is_main and self.should_validate and divisible_by(step, self.val_every_step):
                ...
                total_val_bspline_recon_loss = 0.
                total_val_kl_loss = 0.
                total_val_bspline_control_loss = 0.
                ...
                for _ in range(num_val_batches):
                    with self.accelerator.autocast(), torch.no_grad():
                        ...
                        if isinstance(loss_dict, dict):
                            ...
                            if 'bspline_recon_loss' in loss_dict:
                                total_val_bspline_recon_loss += (loss_dict['bspline_recon_loss'] / num_val_batches)
                            if 'kl_loss' in loss_dict:
                                total_val_kl_loss += (loss_dict['kl_loss'] / num_val_batches)
                            if 'bspline_control_point_loss' in loss_dict:
                                total_val_bspline_control_loss += (loss_dict['bspline_control_point_loss'] / num_val_batches)
                ...
                if total_val_bspline_recon_loss > 0:
                    self.print(get_current_time() + f' valid bspline_recon_loss: {total_val_bspline_recon_loss:.3f}')
                if total_val_kl_loss > 0:
                    self.print(get_current_time() + f' valid kl_loss: {total_val_kl_loss:.3f}')
                if total_val_bspline_control_loss > 0:
                    self.print(get_current_time() + f' valid bspline_control_loss: {total_val_bspline_control_loss:.3f}')
                ...
                if total_val_bspline_recon_loss > 0:
                    val_log_dict["val_bspline_recon_loss"] = total_val_bspline_recon_loss
                if total_val_kl_loss > 0:
                    val_log_dict["val_kl_loss"] = total_val_kl_loss
                if total_val_bspline_control_loss > 0:
                    val_log_dict["val_bspline_control_loss"] = total_val_bspline_control_loss
