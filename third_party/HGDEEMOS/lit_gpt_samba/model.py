...
class GPT(nn.Module):
    ...
    def forward(
        ...
        if use_kv_cache:
            ...
            mask = None  # ← 关键：不传 mask，依赖 FlashAttention 的 causal=True
        else:
            if not self.config.nope:
                cos = cos[:T]
                sin = sin[:T]
        ...
        if not use_kv_cache:
            ...
            # ← 关键：KV cache 长度限制为 local_window
            kv_max_len = min(max_seq_length, self.config.local_window) if self.config.local_window > 0 else max_seq_length
            if self.config.nope:
                self.kv_caches = self.kv_caches or self.build_kv_caches(x, kv_max_len, None)
            else:
                self.kv_caches = self.kv_caches or self.build_kv_caches(x, kv_max_len, cos.size(-1) * 2)
            
            if not hasattr(self, 'inference_params') or self.inference_params is None:
                from flash_attn.utils.generation import InferenceParams
                self.inference_params = InferenceParams(max_seqlen=kv_max_len, max_batch_size=x.size(0))
            ...
            for i, block in enumerate(self.transformer.h):
                x, self.kv_caches[i] = block(x, rope, kv_max_len, mask, input_pos, self.kv_caches[i], self.inference_params, pc=cond_embeds)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_cache is not None:
            ...
            cache_len = cache_k.size(1)  # 应等于 local_window
