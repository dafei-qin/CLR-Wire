...
class GPT(nn.Module):
    ...
    def forward(
        ...
        if not use_kv_cache:
            ...
            # Create inference_params for Mamba layers
            ...
            self.inference_params.seqlen_offset = input_pos[0].item()
            
            for i, block in enumerate(self.transformer.h):
                x, self.kv_caches[i] = block(x, rope, max_seq_length, mask, input_pos, self.kv_caches[i], self.inference_params)
