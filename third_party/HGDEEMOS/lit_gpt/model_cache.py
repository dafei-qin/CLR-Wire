...
class GPTCache(nn.Module):
    def __init__(self, config: Config, build_conditioner: bool = True) -> None:
        ...
        self.cond_embeds: Optional[torch.Tensor] = None  # 初始化 cond_embeds
        self.conditioner = None
        if build_conditioner:
            # 与 model.py 保持一致，使用 config.yaml 参数创建 ShapeVAE
        ...
        # ========== Condition Tokens 降采样配置（与 model.py 保持一致）==========
        self.condition_downsample_factor = 1  # 不降采样，保持 4096 tokens
        self.condition_downsample_method = 'learnable'
        
        shapevae_width = 1024  # ShapeVAE 的 width 参数
    ...
    def forward(
        ...
        if pc is not None:
            # ========== Condition Encoding（与 model.py 保持一致）==========
            # Stage 1: 编码为 latent codes
            latent_codes = self.conditioner.encode(pc)
            # latent_codes: (bs, num_latents, embed_dim) = (bs, 4096, 64)
            
            # Stage 2: 解码为语义特征
            cond_embeds = self.conditioner.decode(latent_codes)
            # cond_embeds: (bs, num_latents, width) = (bs, 4096, 1024)
            ...
            cond_embeds = self.linear(cond_embeds)  # (bs, 4096 or downsampled, n_embd)
            cond_embeds = self.norm(cond_embeds)
            self.cond_embeds = cond_embeds
        else:
            if self.cond_embeds is None:
                raise ValueError("cond_embeds is not initialized. Please provide pc in the first forward call.")
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if self.config.parallel_residual:
            ...
            x = x + h 
            # 检查 pc 是否为有效的 tensor（不仅检查是否为 None，还检查是否为有效的 tensor）
            if pc is not None and (isinstance(pc, torch.Tensor) and pc.numel() > 0):
                x_skip = x
                x = self.norm_cross(x)
