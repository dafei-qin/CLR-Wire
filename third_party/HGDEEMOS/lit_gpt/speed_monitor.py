...
class SpeedMonitorBase:
    ...
    def __init__(
        ...
        self.history_lr: Deque[float] = deque(maxlen=window_size + 1)
        self.history_FWLoss: Deque[float] = deque(maxlen=window_size + 1)
        self.history_cd_loss: Deque[float] = deque(maxlen=log_iter_interval)
        # CD 加权相关：分别记录原始 loss 和加权后的 loss
        self.history_original_loss: Deque[float] = deque(maxlen=log_iter_interval)
        self.history_weighted_loss: Deque[float] = deque(maxlen=log_iter_interval)
        self.history_cd_weight_mean: Deque[float] = deque(maxlen=log_iter_interval)
        self.divider = 1
    ...
    def on_train_batch_end(
        ...
        FWLoss: Optional[float] = None,
        cd_loss: Optional[float] = None,
        # 新增：CD 加权相关参数（可选，向后兼容）
        original_loss: Optional[float] = None,  # 加权前的原始 loss
        weighted_loss: Optional[float] = None,  # 加权后的 loss
        cd_weight_mean: Optional[float] = None,  # CD 权重均值
    ):
        self.iter += 1
        metrics = {}
        ...
        self.history_FWLoss.append(FWLoss)
        self.history_cd_loss.append(cd_loss)
        # CD 加权相关历史记录
        self.history_original_loss.append(original_loss)
        self.history_weighted_loss.append(weighted_loss)
        self.history_cd_weight_mean.append(cd_weight_mean)
        ...
        if len(self.history_wct) == self.history_wct.maxlen:
            ...
            if lengths is not None:
                ...
                if train_loss is not None:
                    ...
                    avg_lr = sum(self.history_lr) / len(self.history_lr)
                    avg_FWLoss = sum(self.history_FWLoss) / len(self.history_FWLoss)
                    metrics.update(
                        {
                            "metric/train_loss": avg_loss,
                            "metric/train_ppl": math.exp(avg_loss),
                            "metric/lr": avg_lr,
                            "metric/FWloss": avg_FWLoss
                        }
                    )
                    # 添加 CD loss（过滤掉 None 值）
                    valid_cd_losses = [cd for cd in self.history_cd_loss if cd is not None]
                    if len(valid_cd_losses) > 0:
                        avg_cd_loss = sum(valid_cd_losses) / len(valid_cd_losses)
                        metrics.update({
                            "metric/cd_loss": avg_cd_loss
                        })
                    
                    # 添加加权前后的 loss 对比（过滤掉 None 值）
                    valid_original_losses = [l for l in self.history_original_loss if l is not None]
                    valid_weighted_losses = [l for l in self.history_weighted_loss if l is not None]
                    valid_cd_weight_means = [w for w in self.history_cd_weight_mean if w is not None]
                    
                    if len(valid_original_losses) > 0:
                        avg_original_loss = sum(valid_original_losses) / len(valid_original_losses)
                        metrics.update({
                            "metric/original_loss": avg_original_loss,
                            "metric/original_ppl": math.exp(avg_original_loss) if avg_original_loss < 20 else float('inf'),
                        })
                    
                    if len(valid_weighted_losses) > 0:
                        avg_weighted_loss = sum(valid_weighted_losses) / len(valid_weighted_losses)
                        metrics.update({
                            "metric/weighted_loss": avg_weighted_loss,
                            "metric/weighted_ppl": math.exp(avg_weighted_loss) if avg_weighted_loss < 20 else float('inf'),
                        })
                    
                    if len(valid_cd_weight_means) > 0:
                        avg_cd_weight_mean = sum(valid_cd_weight_means) / len(valid_cd_weight_means)
                        metrics.update({
                            "metric/cd_weight_mean": avg_cd_weight_mean,
                        })
