...
def visualize_json_interset(cad_data):
    ...
    for face in faces_list:
        print(f"Processing face {face['face_index']} with type {face['surface_type']}")
        surface_type = face['surface_type']
        surface_index = face['face_index']
        if surface_type == 'Plane':
            continue
            occ_face, vertices, faces = build_plane_face(face)
        elif surface_type == 'Cylinder' or surface_type == 'Cone' or surface_type == 'Toroid':
            occ_face, vertices, faces = build_second_order_surface(face)
        # elif surface_type == "Cone":
        #     face_builder = build_cone_face(face)
        # elif surface_type == 'Toroid':
        #     face_builder = build_toroid_face(face)
